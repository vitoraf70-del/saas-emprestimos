...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (body.action === "isentar_juros") {
      ...
      if (parcela.juros_isentos) {
        return NextResponse.json({ error: "Juros de atraso já foram retirados desta parcela." }, { status: 400 });
      }
      ...
      if (calc.jurosValor <= 0) {
        return NextResponse.json({ error: "Esta parcela não possui juros de atraso." }, { status: 400 });
      }
      ...
      const updated = await prisma.parcela.update({
        ...
        data: {
          ...
          juros_valor: 0,
          valor_atualizado: Number(parcela.valor_original) + calc.multaValor,
